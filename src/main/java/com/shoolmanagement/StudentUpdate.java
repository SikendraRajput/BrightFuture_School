...
public class StudentUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		PreparedStatement ps=con.prepareStatement("UPDATE student SET name=?,father=?,mother=?,village=?,dist=?,mobile=?,rollnumber=?,class=?,addmissiondate=? WHERE rollnumber=?");
		...
		if(x<1) {
			...
			pw.println("<h2 style='text-align:center; color:black; position:absolute;top:9px;left:691px; font-size:21px;'>Sorry!! Data Not Updated..</h2>");
		}
		else {
			RequestDispatcher rd=request.getRequestDispatcher("StudentUpdate.jsp");
			rd.include(request, response);
			pw.println("<h2 style='text-align:center; color:black; position:absolute;top:9px;left:691px; font-size:21px;'>Ok!! Data Update..</h2>");
